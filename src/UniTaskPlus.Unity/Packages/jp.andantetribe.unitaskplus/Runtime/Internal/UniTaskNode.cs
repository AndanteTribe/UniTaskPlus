...
{
    /// <summary>
    /// A similar implementation of <see cref="AutoResetUniTaskCompletionSource{T}"/> that supports list structure pooling.
    /// Also supports high-performance timeout control using <see cref="TimeoutController"/>.
    /// </summary>
    internal sealed class UniTaskNode<T> : IUniTaskSource<T>, ITaskPoolNode<UniTaskNode<T>>, IPromise<T>
    {
        ...
        public UniTask<T> WaitAsync(in int millisecondsTimeout, in CancellationToken cancellationToken = default)
        {
            if (cancellationToken.CanBeCanceled)
            {
                ...
                _externalCancellationRegistration = cancellationToken.RegisterWithoutCaptureExecutionContext(static state =>
                {
            ...
            if (millisecondsTimeout != Timeout.Infinite)
            {
                ...
                _timeoutRegistration = _timeoutToken.RegisterWithoutCaptureExecutionContext(static state =>
                {
                    var self = (UniTaskNode<T>)state!;
